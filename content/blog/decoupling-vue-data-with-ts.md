...
但后面折腾半天我终于知道了  
**这种做法存在一个根本问题：**  
即使我不把新的组件（ProductData.vue）挂载到 app 上让它不显示，可是不被挂载的组件**里面提供的所有东西都是不生效的**  
...
#Vue #TypeScript #前端架构 #代码解耦

**本文记录了一位初学者在探索 Vue 组件与数据解耦过程中的思考与实践。文章通过对比“使用未挂载组件”的错误思路与“使用纯 TypeScript 模块”的正确方案，清晰地展示了后者在维护性上的优势，并点出了在 TS 模块中处理静态资源路径的常见“陷阱”与解决方案。**

1.  **解耦动机与误区 (Motivation & Misconception)**
    -   目标：寻找一种方法，将组件的视图与数据逻辑解耦，避免通过父组件进行繁琐的数据 (`props`) 传递。
    -   错误尝试：最初设想创建一个独立的 Vue 组件来承载数据，但不将其挂载到 DOM。实践证明此路不通，因为 Vue 组件未被挂载，其内部逻辑不会执行。
2.  **正确方案与实践 (Correct Approach & Practice)**
    -   解决方案：转向使用纯 TypeScript 模块 (`.ts` 文件)。在 TS 文件中定义并导出响应式数据对象 (`reactive`)。
    -   组件消费：在需要数据的 Vue 组件中，直接 `import` 该 TS 模块即可使用，实现了清晰的关注点分离。
    -   路径问题：指出了在 TS 模块中使用相对路径引用图片等静态资源会失败，最稳定的方案是将资源放在 `public` 目录下并使用根绝对路径（如 `/images/my-image.png`）。
